...
# Launch IIS Express, serving files from the Breeze Samples DocCode directory iin a well-known location
$scriptDir = (Split-Path -Parent $MyInvocation.MyCommand.Definition)

# When in breeze.js.samples 
$wwwRoot = (Resolve-Path $scriptDir\..\..\net\DocCode\DocCode).Path

Push-Location -Path (Split-Path -Parent $iisExpress)
cmd /c iisexpress.exe /port:58066 /path:$wwwRoot
Pop-Location