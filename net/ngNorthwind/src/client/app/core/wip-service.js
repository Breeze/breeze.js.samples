...
(function (){
    'use strict';
    angular.module('app').factory('wip-service', WipService);

    WipService.$inject = ['$rootScope','$timeout','$window', 'breeze', 'logger'];

    function WipService($rootScope, $timeout, $window, breeze, logger){
        ...
        function initialize(entityManager){
            ...
            if (!db){
                disabled = true;
                stashCount = 0;
                logger.error('Browser does not support local storage; WIP disabled.');
            } else {
        ...
        function restore(){
            ...
            try {
                ...
                logger.error('WIP restore failed');
                logger.error(error);
            } finally {
                isRestoring = false;
            }
