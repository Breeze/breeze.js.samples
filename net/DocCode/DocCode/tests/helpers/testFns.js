...
docCode.testFns = (function () {
    ...
    *********************************************************/
    ...
    /*********************************************************
    * Teardowns for modules that save to the database
    * Should call during test teardown to restore
    ...
    *********************************************************/
    ...
    function teardown_northwindReset() {
      stop();
      northwindReset(true) // jQuery promise
        .fail(handleFail).always(start);
    }

    function northwindReset(fullReset) {
      var queryString = fullReset ? "/?options=fullreset" : "";
      var command = "/reset"+ queryString;
      var config = {headers: { "X-UserSessionId": testFns.userSessionId }};
      return webApiCommand(testFns.northwindServiceName, command, config);
    }

    /// Todos ///
    function teardown_todosReset() {
      stop();
      todosReset() // jQuery promise
        .fail(handleFail).always(start);
    }

    function todosPurge() {
      return webApiCommand(testFns.todosServiceName, 'purge');
    }

    function todosReset() {
      return webApiCommand(testFns.todosServiceName, 'reset');
    }
