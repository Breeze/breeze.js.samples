/*
 * Thomas Burleson's Q-infused version of mongoQuery.js from the breeze-mongodb
 * This is really server-side code. Holding onto it for review
 */
var mongodb = require('mongodb');
var ObjectId = require('mongodb').ObjectID;
var Q = require( 'q' );

// UNCOMMENT when moved back into server
...
/**
 * Query Execute() API that support both callbacks and promises
 * @returns Promise for optional use (if callback is also provided)
 */
MongoQuery.prototype.execute = function(db, collectionName, fn)
{
    ...
    db.collection(collectionName, {strict: true} , function (err, collection)
    {
        ...
        if (err)
        {
            dfd.reject({
                statusCode: 404,
                message: "Unable to locate: " + collectionName,
                error: err
            });
        }
        else if (that.inlineCount)
        {
            collection.count(that.filter, function(err, count)
            {
                ...
                if (that.options && that.options.limit === 0) {
                    dfd.resolve({ Results: [], InlineCount: count });
                    return;
                }
                ...
                src.toArray(function (err, results) {
                    ...
                    dfd.resolve({ Results: results, InlineCount: count });
                });
            });
        } else {
